...
This message type is used both for ordinary data writes and for parity writes. Importantly, the disk process itself is unaware of RAID semantics: parity computation is done entirely in the controller, which first reads the old data block and old parity block, computes `new_parity = old_parity XOR old_data XOR new_data`, writes the updated parity block, and then writes the new data block. Thus, the protocol seen by the child remains a simple block-write protocol. :contentReference[oaicite:4]{index=4} :contentReference[oaicite:5]{index=5}
